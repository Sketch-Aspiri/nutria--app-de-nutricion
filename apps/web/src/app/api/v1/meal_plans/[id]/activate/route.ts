...
/**
 * POST /api/v1/meal_plans/{id}/activate — valida alergias, archiva el plan
 * activo anterior y activa este, sin marcarlo todavía como compartido.
 */
export async function POST(_request: Request, { params }: Contexto) {
  ...
  try {
    const plan = await activarPlan(sesion.userId, id);
    ...
    logger.error('Falló la activación del plan alimenticio', error);
